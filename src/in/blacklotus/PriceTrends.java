...
public class PriceTrends {
	...
	private static final String[] SORT_KEYS = { "PRICE", "VOLUME", "DAY" };
	...
	private static void processPriceTrendData(List<Symbol> symbolList) {
		...
		String[] headers = new String[] { "#", "SYMBOL", "LOW20", "HIGH20", TREND.toUpperCase() + "TREND", "$CHANGE",
				"%$CHANGE", "PriR", "VOLUME", "%VOLCHANGE", "VolR", "DayR" };
		...
		for (int i = 0; i < processedTrendList.size(); i++) {
			...
			if ("PRICE".equals(SORT_KEY)) {

				processedTrendList.get(i).sortByPriceRank();

			} else if ("VOLUME".equals(SORT_KEY)) {

				processedTrendList.get(i).sortByVolumeRank();

			} else if ("DAY".equals(SORT_KEY)) {

				processedTrendList.get(i).sortByDaysRank();
			}
	...
	public static PriceTrend getTrendDetails(String stockName) {
		...
		try {
			...
			trend.setTrend(getPriceTrend(closeValues, trend.getLow(), trend.getLowDate(), trend.getHigh(),
	...
	private static List<PriceTrendData> getPriceTrend(Double closeValues[], Double low20, Date lowDate, Double high20,
